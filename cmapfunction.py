...
def main_statement(i):
        if(tag[i]=='variable' or tag[i]=='constant' or tag[i]=='operator'):
            f.write(text[i])
        if(tag[i]=='print'):
            if(text[i] in var_name):
                f.write('printf(\"')
                f.write('%')
                if(var_type[var_name.index(text[i])]=='int'):
                    f.write('d\",')
                if(var_type[var_name.index(text[i])]=='float'):
                    f.write('f\",')
                if(var_type[var_name.index(text[i])]=='char'):
                    f.write('c\",')    
                f.write(text[i])
                f.write('");\n')
            else:    
        ...
        if(tag[i]=='log'):
            ...
            f.write(text[i+1])
            f.write(');')       
        if(tag[i]=='file-pointer'):
            f.write('FILE *')
            f.write(text[i])
        if(tag[i]=='fopen'):
            f.write('fopen(\"')
            f.write(text[i+1]) #file-name
            f.write('\",')
            f.write(text[i+2]) #file-mode
            f.write(');\n')
        if(tag[i]=='fgetc'):
            f.write('fgetc(')
            f.write(text[i]) #file-pointer
            f.write(')')
        if(tag[i]=='fclose'):
            f.write('fclose(')
            f.write(text[i]) #file-pointer
